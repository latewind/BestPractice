...
//		new PuzzleSolver().compare();
	...
	public class Puzzle{
		...
		public boolean reachTheEnd(Point point){
			...
			if(point.getX()==endPoint.getX()&&point.getY()==endPoint.getY())
				return true;
			else
				return false;
		}
	...
	public 	List<Point> nextPoints(){
		...
		boolean isValid(Point point){
			if(isFather(point)||outOfBound(point)||isNot(point))
				return false;
			else
				return true;
		}
		private boolean  isFather(Point point){
			...
			if(point.getX()==father.getX()&&point.getY()==father.getY())
				return true;
			else
				return false;			
		}
		private boolean outOfBound(Point point){
			if(point.getX()<0||point.getY()<0||point.getX()>map.length-1||point.getY()>map[0].length-1)
				return true;
			else
				return false;
		}
